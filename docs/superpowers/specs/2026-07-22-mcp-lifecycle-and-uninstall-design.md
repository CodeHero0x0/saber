...
- 用户全局 MCP 安装。
- OAuth MCP 认证不在 MVP 范围内；需要 OAuth 的 server 必须等待后续受控代理方案。
- 自动迁移旧配置或旧运行清单。
...
`stdio` server 允许 `id`、`transport`、`command`、`args`、`cwd`、`env`、`tools`。HTTP server 允许 `id`、`transport`、`url`、`headers`、`tools`。两种 transport 不得混用对方专属字段。HTTP 统一通过 Saber bridge 连接；`auth: none` 和 `auth: oauth` 都是明确拒绝的未知字段。
...
每个 server 在工具原生配置中表现为独立的 `saber--<server-id>` stdio MCP server。它启动 Saber 的 TypeScript bridge，并指向本次物化生成的无密钥运行描述文件。
...
MVP 不接受 OAuth server。HTTP server 同样必须经过 Saber bridge，避免工具原生直连绕过 capability 映射和 L2/L3 风险边界。
...
preview token 由随机 nonce 生成并绑定排序后的完整删除计划、目标、当前内容指纹和配置来源指纹。执行前重新计算计划；任何变化使 token 失效。token 通过本地 `0600` preview record 原子单次消费，已消费 token 即使同配置重装也不能重放。
...
materialize 与 uninstall 共用仓库级生命周期锁。取得锁后先全局校验并恢复严格命名、绑定 operation/tool/target 且路径限于对应作用域的遗留事务；恢复完成前禁止新写。执行时先在 `.saber/runtime/transactions/` 保存恢复数据，再移除精确条目。任一步失败恢复所有已变更目标，避免半卸载。
...
- 工具配置是否需要项目信任或重启，以及是否存在不受支持的 OAuth 配置。
- 是否存在未恢复事务。
...
- `stdio`、HTTP header、团队 server、个人 server 和显式选择；OAuth schema 必须被拒绝。
