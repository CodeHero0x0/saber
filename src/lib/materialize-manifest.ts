...
export function parseRuntimeManifest(text: string): RuntimeManifest {
  ...
  if (!isRecord(value) || !exactKeys(value, [
    "schemaVersion", "managedBy", "tool", "capabilities", "coreCommands",
    "teamSkills", "prompts", "externalSkills", "projections", "mcpServers", "mcpEntries",
    "toolConfig", "sourceFingerprints",
  ]) || value.schemaVersion !== 6 || value.managedBy !== "saber"
    ...
    || !stringArray(value.prompts) || !stringArray(value.externalSkills)
  ...
  const uniqueLists = [
    ...
    manifest.externalSkills, manifest.mcpServers,
