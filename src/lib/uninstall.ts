...
export type UninstallSelection = { all?: boolean; tool?: ToolName };
...
export type UninstallPlan = {
  schemaVersion: 1;
  operation: "uninstall";
  selection: { all: boolean; tool: ToolName | null };
...
function selectionOf(selection: UninstallSelection): UninstallPlan["selection"] {
  ...
  if (all && selection.tool !== undefined) throw new SaberError("--all cannot be combined with --tool", 2);
  if (!all && selection.tool === undefined) throw new SaberError("--tool is required unless --all is used", 2);
  return { all, tool: selection.tool ?? null };
}
async function manifestPaths(root: string, selection: UninstallPlan["selection"]): Promise<string[]> {
  if (!selection.all) return [`.saber/runtime/materialize/${selection.tool}/root.json`];
  const result: string[] = [];
...
function assertRootManifest(manifest: RuntimeManifest): void {
  ...
  if (configPath !== adapterPath) throw new SaberError("uninstall manifest does not match its root target", 2);
}
...
async function prepareTarget(root: string, manifestPath: string, selection: UninstallPlan["selection"]): Promise<Prepared | undefined> {
  ...
  const expected = `.saber/runtime/materialize/${manifest.tool}/root.json`;
  if (manifestPath !== expected) throw new SaberError("materialize runtime manifest does not match its managed path", 2);
  if (!selection.all && manifest.tool !== selection.tool) throw new SaberError("materialize runtime manifest does not match the selected target", 2);
  ...
  for (const projection of manifest.projections) {
    ...
    const path = projection.linkPath;
    await assertSafeParents(root, path);
  ...
  return {
    plan: {
      tool: manifest.tool, manifestPath,
