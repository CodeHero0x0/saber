...
function validateSkillSet(input: RepositoryValidationInput, errors: string[]): void {
  if (input.skillSet === undefined) {
    return;
  }
  ...
  for (const duplicate of findDuplicateValues(input.skillSet.team)) {
    errors.push(`duplicate team skill ${duplicate}`);
  }
  for (const duplicate of findDuplicateValues(input.skillSet.external)) {
    errors.push(`duplicate external skill ${duplicate}`);
  }
  for (const skillId of input.skillSet.external) {
    if (!selectedExternalSkills.has(skillId)) {
      errors.push(`skill set references unknown external skill ${skillId}`);
    }
  }
}
