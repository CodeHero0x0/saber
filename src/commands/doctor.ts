...
import { missingMcpEnvironment } from "../lib/mcp-environment.js";
...
export async function collectDoctorReport(
  ...
  const mcpServers = await Promise.all(
    [...(config.mcp?.servers ?? []), ...(config.local?.mcp?.servers ?? [])].map(async (server) => {
      ...
      return {
        ...
        environment: {
          ...
          missing: await missingMcpEnvironment(repositoryRoot, uniqueDeclared),
        },
      };
    }),
  );
