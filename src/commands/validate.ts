...
import { validateKnowledgeAssets } from "../lib/knowledge.js";
...
/** Check that the globally materialized team skills exist before init. */
export async function validateSkillSetAssetReferences(
  repositoryRoot: string,
...
/** Validate configuration and all checked-in `/saber` skill contracts. */
export async function collectValidationReport(
  ...
  if (loadedConfig?.skillSet !== undefined) {
    errors.push(...(await validateSkillSetAssetReferences(repositoryRoot, loadedConfig.skillSet.team)));
  }
  ...
  try {
    errors.push(...(await validateKnowledgeAssets(repositoryRoot)));
  } catch {
    errors.push("could not validate project knowledge assets");
  }
