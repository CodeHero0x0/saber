...
async function configWith(externalAssets: ExternalAssetsConfig) {
  ...
  // fixtures so each test exercises only the external asset contract.
  return { ...config, externalAssets, skillSet: { team: [], external: [] } };
}
    ...
    assert.deepEqual(commands, [
      {
        ...
        args: [
          "-C",
          join(realpathSync(root), ".saber/cache/saber-v1/superpowers"),
          "remote",
          "get-url",
          "origin",
        ],
