...
test("MCP schema rejects duplicate IDs, tools, and explicit selections", async () => {
...
test("MCP schema allows multiple read-only tools to share one capability", async () => {
  const source = `${teamPrefix}mcp:
  servers:
    - id: mysql
      transport: stdio
      command: npx
      args: [-y, "@database-mcp/mysql@0.4.2"]
...
`;
  await withConfig(source, undefined, async (root) => {
    const config = await loadRepositoryConfig(root);
    assert.equal(config.mcp.servers[0]?.tools.length, 2);
  });
});
...
test("checked-in team configuration enables the MySQL MCP and personal example adds none", async () => {
  ...
  assert.deepEqual(config.mcp.servers, [{
    ...
    env: ["MYSQL_HOST", "MYSQL_PORT", "MYSQL_USER", "MYSQL_PASSWORD", "MYSQL_DATABASE"],
    tools: [
      { name: "execute_sql", capability: "mysql.read" },
      { name: "search_objects", capability: "mysql.read" },
    ],
  }]);
