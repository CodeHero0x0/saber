...
`workitems/<ID>.md`。它优先使用 `superpowers` 的最小相关方法：
...
Superpowers 方法的结论和证据必须写回当前工作项：需求变化记录 `REQ-CHG-*`，计划变化记录
`PLAN-REV-*`，实现、测试、QA 发现和交付总结分别写入正文相应部分。不要把方法自身建议的额外目录
或文档当作正式工作项产物，除非用户明确要求。
