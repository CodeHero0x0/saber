...
3. 用户确认之后，才允许把确认草稿写入仓库内受控的临时 Markdown 文件，并由后台调用内部 CLI 的 `--source-file` 文件接口创建工作项。
4. 不把短文本或 `--source-text` 传给 CLI/命令行，不保存或落盘完整聊天；工作项只保留经确认的输入快照、简要澄清结论和必要引用。

所有来源共用一套 `source` 模型：`chat | jira | document | manual`。Jira 仅表示 `source.kind: jira`；其他类型不得虚构 Jira 编号。复杂、矛盾或高影响内容可建议用户显式输入 `/grilling`，但本技能不能替用户触发。
