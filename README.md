...
读取和写入结果会以脱敏后的 `data` 返回；GitLab 创建遇到 409 时不会猜测是否成功，而是提示使用源分支查询恢复已有 MR。

HTTP token 只在执行阶段读取。预览会绑定非敏感服务目标的摘要，并显示 credential 变量名、业务目标、HTTP 方法和脱敏后的变更；不会显示 token 或完整服务地址。
...
| L2 | Jira 更新、创建 MR、MCP 写入；手工 push 也按 L2 管理 | 自动动作先 preview，必须精确 token 和真人确认 |
...
MVP 不包含 L3 操作、自动外部更新、自治多 Agent 编排、中央权限审计平台、测试平台直连、第三种以上 AI 工具的专属适配、自动 push、merge 或 deploy。代码提交和 push 仍由成员在对应业务仓中按团队 Git 规则执行；Saber 负责安全预览的 Jira/GitLab HTTP 写入和跨仓证据记录。先用至少三个真实需求验证 BA → Dev → QA → 多仓 MR 的黄金路径，再扩展能力。
